...
ChatSession::ChatSession(const std::shared_ptr<TcpConnection>& conn, int sessionid)
    : TcpSession(conn), m_id(sessionid), m_seq(0), m_isLogin(false)
{
    m_userinfo.userid = 0;
...
ChatSession::~ChatSession()
{
    ...
    if (conn) {
        LOG_INFO("remove check online timerId, userid: %d, clientType: %d, client address: %s", m_userinfo.userid,
...
void ChatSession::onRead(const std::shared_ptr<TcpConnection>& conn, ByteBuffer* pBuffer, Timestamp receivTime)
{
    while (true) {
        ...
        if (header.compressflag == PACKAGE_COMPRESSED) {
            // 包头有错误，立即关闭连接
            if (header.compresssize <= 0 || header.compresssize > MAX_PACKAGE_SIZE || header.originsize <= 0 ||
                header.originsize > MAX_PACKAGE_SIZE) {
                // 客户端发非法数据包，服务器主动关闭之
                LOG_ERROR("Illegal package, compresssize: %lld, originsize: %lld, close TcpConnection, client: %s",
            ...
            std::string inbuf;
            inbuf.append(pBuffer->peek(), static_cast<std::size_t>(header.compresssize));
            pBuffer->retrieve(static_cast<size_t>(header.compresssize));
            std::string destbuf;
            if (!ZlibUtil::uncompressBuf(inbuf, destbuf, static_cast<size_t>(header.originsize))) {
        ...
        // 数据包未压缩
            ...
            pBuffer->retrieve(sizeof(chat_msg_header));
            std::string inbuf;
            inbuf.append(pBuffer->peek(), static_cast<size_t>(header.originsize));
            pBuffer->retrieve(static_cast<size_t>(header.originsize));
...
bool ChatSession::process(const std::shared_ptr<TcpConnection>& conn, const char* inbuf, size_t buflength)
{
    ...
    if (!readStream.ReadString(&data, 0, datalength)) {
        LOG_ERROR("read data error, client: %s", conn->peerAddress().toIpPort().c_str());
        return false;
    }

    // 心跳包太频繁，不打印
    if (cmd != msg_type_heartbeat)
        LOG_INFO("Request from client: userid=%d, cmd=%d, seq=%d, data=%s, datalength=%d, buflength=%d",
                 m_userinfo.userid, cmd, m_seq, data.c_str(), datalength, buflength);

    if (Singleton<ChatServer>::Instance().isLogPackageBinaryEnabled()) {
        LOG_INFO("body stream, buflength: %d, client: %s", buflength, conn->peerAddress().toIpPort().c_str());
    ...
    switch (cmd) {
        ...
        default: {
            if (m_isLogin) {
                switch (cmd) {
                    ...
                    case msg_type_chat: {
                        ...
                        if (!readStream.ReadInt32(target)) {
                            LOG_ERROR("read target error, client: %s", conn->peerAddress().toIpPort().c_str());
                            return false;
                        }
                    ...
                    case msg_type_multichat: {
                        ...
                        if (!readStream.ReadString(&targets, 0, targetslength)) {
                            LOG_ERROR("read targets error, client: %s", conn->peerAddress().toIpPort().c_str());
                            return false;
                        }
                    ...
                    case msg_type_remotedesktop: {
                        ...
                        if (!readStream.ReadString(&bmpHeader, 0, bmpHeaderlength)) {
                            LOG_ERROR("read bmpheader error, client: %s", conn->peerAddress().toIpPort().c_str());
                            return false;
                        }
                        ...
                        if (!readStream.ReadString(&bmpData, 0, bmpDatalength)) {
                            LOG_ERROR("read bmpdata error, client: %s", conn->peerAddress().toIpPort().c_str());
                            return false;
                        }
                    ...
                    case msg_type_updateteaminfo: {
                        ...
                        if (!readStream.ReadInt32(operationType)) {
                            LOG_ERROR("read operationType error, client: %s", conn->peerAddress().toIpPort().c_str());
                            return false;
                        }
                        ...
                        if (!readStream.ReadString(&newTeamName, 0, newTeamNameLength)) {
                            LOG_ERROR("read newTeamName error, client: %s", conn->peerAddress().toIpPort().c_str());
                            return false;
                        }
                    ...
                    case msg_type_modifyfriendmarkname: {
                        ...
                        if (!readStream.ReadInt32(friendid)) {
                            LOG_ERROR("read friendid error, client: %s", conn->peerAddress().toIpPort().c_str());
                            return false;
                        }
                        ...
                        if (!readStream.ReadString(&newmarkname, 0, newmarknamelength)) {
                            LOG_ERROR("read newmarkname error, client: %s", conn->peerAddress().toIpPort().c_str());
                            return false;
                        }
                    ...
                    case msg_type_movefriendtootherteam: {
                        ...
                        size_t oldteamnamelength;
                        if (!readStream.ReadString(&oldteamname, 0, oldteamnamelength)) {
                            LOG_ERROR("read oldteamname error, client: %s", conn->peerAddress().toIpPort().c_str());
                            return false;
                        }
                    ...
                    default:
                        // pBuffer->retrieveAll();
                        LOG_ERROR("unsupport cmd, cmd: %d, data=%s, connection name:", cmd, data.c_str(),
                ...
                std::string responseData = "{\"code\": 2, \"msg\": \"not login, please login first!\"}";
                send(cmd, m_seq, responseData);
                LOG_INFO("Response to client: cmd=%d, , data=%s, , sessionId=%d", cmd, responseData.c_str(), m_id);
            } // end if
        } // end default
    } // end outer-switch
...
void ChatSession::onLoginResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    if (!ok || errs.size() != 0) {
        LOG_ERROR("onLoginResponse failed, invalid json: %s, sessionId: %d, client: %s", data.c_str(), m_id,
                  conn->peerAddress().toIpPort().c_str());
        delete reader;
        return;
    }
    ...
    if (!jsonRoot["username"].isString() || !jsonRoot["password"].isString() || !jsonRoot["clienttype"].isInt() ||
        ...
        LOG_ERROR("invalid json: %s, sessionId: %d, client: %s", data.c_str(), m_id,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    if (cachedUser.userid == 0) {
        ...
        if (cachedUser.password != password)
            ...
            if (targetSession) {
                ...
                LOG_INFO("Response to client, userid: %d, cmd=msg_type_kickuser", targetSession->getUserId());
            ...
            os << "{\"code\": 0, \"msg\": \"ok\", \"userid\": " << m_userinfo.userid << ",\"username\":\""
               << cachedUser.username << "\", \"nickname\":\"" << cachedUser.nickname
               << "\", \"facetype\": " << cachedUser.facetype << ", \"customface\":\"" << cachedUser.customface
               << "\", \"gender\":" << cachedUser.gender << ", \"birthday\":" << cachedUser.birthday
               << ", \"signature\":\"" << cachedUser.signature << "\", \"address\": \"" << cachedUser.address
    ...
    LOG_INFO("Response to client: cmd=msg_type_login, data=%s, userid=", os.str().c_str(), m_userinfo.userid);
    ...
    for (const auto& iter : friends) {
        ...
        for (auto& iter2 : sessions) {
            if (iter2) {
                ...
                LOG_INFO("sendUserStatusChangeMsg to user(userid: %d): user go online, online userid: %d, status: %d",
...
void ChatSession::onGetFriendListResponse(const std::shared_ptr<TcpConnection>& conn)
{
    ...
    LOG_INFO("Response to client: userid: %d, cmd=msg_type_getofriendlist, data: %s", m_userinfo.userid,
             os.str().c_str());
}
...
void ChatSession::onChangeUserStatusResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    bool                    ok = reader->parse(data.c_str(), data.c_str() + data.length(), &jsonRoot, &errs);
    if (!ok || errs.size() != 0) {
        LOG_ERROR("invalid json: %s, userid: %d, client: %s", data.c_str(), m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        delete reader;
        return;
    }
    ...
    if (m_userinfo.status == newstatus) return;
    ...
    for (const auto& iter : friends) {
        ...
        imserver.getSessionsByUserId(sessions, iter.userid);
        for (auto& iter2 : sessions) {
            if (iter2) iter2->sendUserStatusChangeMsg(m_userinfo.userid, 1, newstatus);
        }
    }
}
...
void ChatSession::onFindUserResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    if (!Singleton<UserManager>::Instance().getUserInfoByUsername(username, cachedUser))
        ...
        snprintf(szUserInfo, 256,
                 "{ \"code\": 0, \"msg\": \"ok\", \"userinfo\": [{\"userid\": %d, \"username\": \"%s\", \"nickname\": "
                 "\"%s\", \"facetype\":%d}] }",
    ...
    LOG_INFO("Response to client: userid: %d, cmd=msg_type_finduser, data: %s", m_userinfo.userid, retData.c_str());
}
...
void ChatSession::onOperateFriendResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    if (targetUserid >= GROUPID_BOUBDARY) {
        ...
        if (Singleton<UserManager>::Instance().isFriend(m_userinfo.userid, targetUserid)) {
            LOG_ERROR("In group already, unable to join in group, groupid: %d, , userid: %d, , client: %s",
                      targetUserid, m_userinfo.userid, conn->peerAddress().toIpPort().c_str());
            // TODO: 通知下客户端
            return;
        }
    ...
    if (type == 1) {
        if (Singleton<UserManager>::Instance().isFriend(m_userinfo.userid, targetUserid)) {
            LOG_ERROR("Friendship already, unable to add friend, friendid: %d, userid: %d, client: %s", targetUserid,
        ...
        //{"userid": 9, "type": 1, }
        snprintf(szData, 256, "{\"userid\":%d, \"type\":2, \"username\": \"%s\"}", m_userinfo.userid,
    ...
    // 应答加好友
        ...
        if (accept == 1) {
            if (!Singleton<UserManager>::Instance().makeFriendRelationshipInDB(targetUserid, m_userinfo.userid)) {
                LOG_ERROR("make relationship error: %s, userid: %d, client:  %s", data.c_str(), m_userinfo.userid,
                          conn->peerAddress().toIpPort().c_str());
                return;
            }
            ...
            if (!Singleton<UserManager>::Instance().updateUserRelationshipInMemory(m_userinfo.userid, targetUserid,
                ...
                LOG_ERROR("UpdateUserTeamInfo error: %s, , userid: %d, client: %s", data.c_str(), m_userinfo.userid,
                          conn->peerAddress().toIpPort().c_str());
                return;
            }
        }
        ...
        snprintf(szData, 256, "{\"userid\": %d, \"type\": 3, \"username\": \"%s\", \"accept\": %d}", m_userinfo.userid,
                 m_userinfo.username.c_str(), accept);

        // 提示自己当前用户加好友成功
        User targetUser;
        if (!Singleton<UserManager>::Instance().getUserInfoByUserId(targetUserid, targetUser)) {
            LOG_ERROR("Get Userinfo by id error, targetuserid: %d, userid: %d, data: %s, client: %s", targetUserid,
    ...
    if (sessions.empty()) {
        ...
        LOG_INFO("userid: %d, is not online, cache notify msg, msg: %s", targetUserid, outbuf.c_str());
        return;
    }
    ...
    LOG_INFO("Response to client: userid: %d, cmd=msg_type_addfriend, data: %s", targetUserid, data.c_str());
}
...
void ChatSession::onAddGroupResponse(int32_t groupId, const std::shared_ptr<TcpConnection>& conn)
{
    if (!Singleton<UserManager>::Instance().makeFriendRelationshipInDB(m_userinfo.userid, groupId)) {
        LOG_ERROR("make relationship error, groupId: %d, userid: %d, client: %s", groupId, m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    if (!Singleton<UserManager>::Instance().getUserInfoByUserId(groupId, groupUser)) {
        LOG_ERROR("Get group info by id error, targetuserid: %d, userid: %d, client: %s", groupId, m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    char szSelfData[256] = {0};
    ...
    if (!Singleton<UserManager>::Instance().updateUserRelationshipInMemory(m_userinfo.userid, groupId,
                                                                           FRIEND_OPERATION_ADD)) {
        LOG_ERROR("UpdateUserTeamInfo error, targetUserid: %d, userid: %d, client: %s", groupId, m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    for (const auto& iter : friends) {
        ...
        for (auto& iter2 : targetSessions) {
            if (iter2) iter2->sendUserStatusChangeMsg(groupId, 3);
        }
    }
}
...
void ChatSession::onUpdateUserInfoResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    if (!jsonRoot["nickname"].isString() || !jsonRoot["facetype"].isInt() || !jsonRoot["customface"].isString() ||
        !jsonRoot["gender"].isInt() || !jsonRoot["birthday"].isInt() || !jsonRoot["signature"].isString() ||
        !jsonRoot["address"].isString() || !jsonRoot["phonenumber"].isString() || !jsonRoot["mail"].isString()) {
        LOG_ERROR("invalid json: %s, userid: %d, client: %s", data.c_str(), m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    if (!Singleton<UserManager>::Instance().updateUserInfoInDb(m_userinfo.userid, newuserinfo)) {
        ...
        */
        currentuserinfo << "\"userid\": " << m_userinfo.userid << ",\"username\":\"" << m_userinfo.username
                        << "\", \"nickname\":\"" << newuserinfo.nickname << "\", \"facetype\": " << newuserinfo.facetype
                        << ", \"customface\":\"" << newuserinfo.customface << "\", \"gender\":" << newuserinfo.gender
                        << ", \"birthday\":" << newuserinfo.birthday << ", \"signature\":\"" << newuserinfo.signature
                        << "\", \"address\": \"" << newuserinfo.address << "\", \"phonenumber\": \""
                        << newuserinfo.phonenumber << "\", \"mail\":\"" << newuserinfo.mail;
    ...
    LOG_INFO("Response to client: userid: %d, cmd=msg_type_updateuserinfo, data: %s", m_userinfo.userid,
             retdata.str().c_str());
    ...
    for (const auto& iter : friends) {
        ...
        imserver.getSessionsByUserId(targetSessions, iter.userid);
        for (auto& iter2 : targetSessions) {
            if (iter2) iter2->sendUserStatusChangeMsg(m_userinfo.userid, 3);
        }
    }
}
...
void ChatSession::onModifyPasswordResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    if (!Singleton<UserManager>::Instance().getUserInfoByUserId(m_userinfo.userid, cachedUser)) {
        LOG_ERROR("get userinfo error, userid: %d, data: %s, client: %s", m_userinfo.userid, data.c_str(),
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    if (cachedUser.password != oldpass) {
        ...
        if (!Singleton<UserManager>::Instance().modifyUserPassword(m_userinfo.userid, newPass)) {
            ...
            LOG_ERROR("modify password error, userid: %d, data:%s, client: %s", m_userinfo.userid, data.c_str(),
                      conn->peerAddress().toIpPort().c_str());
        } else
    ...
    LOG_INFO("Response to client: userid: %d, cmd=msg_type_modifypassword, data: %s", m_userinfo.userid, data.c_str());
}
...
void ChatSession::onCreateGroupResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    if (!Singleton<UserManager>::Instance().addGroup(groupname.c_str(), m_userinfo.userid, groupid)) {
        LOG_ERROR("Add group error, data: %s, userid: %d, client: %s", data.c_str(), m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
    ...
    // 创建成功以后该用户自动加群
    if (!Singleton<UserManager>::Instance().makeFriendRelationshipInDB(m_userinfo.userid, groupid)) {
        LOG_ERROR("join in group, errordata: %s, userid: %d, client: %s", data.c_str(), m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    if (!Singleton<UserManager>::Instance().updateUserRelationshipInMemory(m_userinfo.userid, groupid,
                                                                           FRIEND_OPERATION_ADD)) {
        LOG_ERROR("UpdateUserTeamInfo error, data: %s, userid: %d, client: %s", data.c_str(), m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    //{
    //     LOG_ERROR << "UpdateUserTeamInfo error, data: " << data << ", userid: " << m_userinfo.userid << ", client: "
    //     << conn->peerAddress().toIpPort(); return;
    // }

    // 应答客户端，建群成功
    send(msg_type_creategroup, m_seq, retdata.str());

    LOG_INFO("Response to client: userid: %d, cmd=msg_type_creategroup, data: %s", m_userinfo.userid,
             retdata.str().c_str());
...
void ChatSession::onGetGroupMembersResponse(const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    for (const auto& iter : friends) {
        ...
        */
        ...
        osSingleUserInfo << "{\"userid\": " << iter.userid << ", \"username\":\"" << iter.username
                         << "\", \"nickname\":\"" << iter.nickname << "\", \"facetype\": " << iter.facetype
                         << ", \"customface\":\"" << iter.customface << "\", \"gender\":" << iter.gender
                         << ", \"birthday\":" << iter.birthday << ", \"signature\":\"" << iter.signature
                         << "\", \"address\": \"" << iter.address << "\", \"phonenumber\": \"" << iter.phonenumber
                         << "\", \"mail\":\"" << iter.mail << "\", \"clienttype\": 1, \"status\":" << useronline << "}";
    ...
    LOG_INFO("Response to client: userid: %d, cmd=msg_type_getgroupmembers, data: %s", m_userinfo.userid,
             os.str().c_str());
}
...
void ChatSession::sendUserStatusChangeMsg(int32_t userid, int type, int status /* = 0*/)
{
    ...
    LOG_INFO("send to client: userid: %d, cmd=msg_type_userstatuschange, data: %s", m_userinfo.userid, data.c_str());
}
...
void ChatSession::onChatResponse(int32_t targetid, const std::string& data, const std::shared_ptr<TcpConnection>& conn)
{
    ...
    if (!modifyChatMsgLocalTimeToServerTime(data, modifiedChatData)) {
        LOG_ERROR("invalid chat json, chatjson: %s, senderid: %d, targetid: %d, chatmsg: %s, client: %s", data.c_str(),
    ...
    if (!userMgr.saveChatMsgToDb(m_userinfo.userid, targetid, data)) {
        LOG_ERROR("Write chat msg to db error, senderid: %d, targetid: %d, chatmsg: %s, client: %s", m_userinfo.userid,
                                      ...
                                      const std::shared_ptr<TcpConnection>& conn)
{
    ...
    if (!ok || errs.size() != 0) {
        LOG_ERROR("invalid targets string: targets: %s, data: %s, userid: %d, , client: %s", targets.c_str(),
                  data.c_str(), m_userinfo.userid, conn->peerAddress().toIpPort().c_str());
        delete reader;
        return;
    }
    delete reader;
    ...
    if (!jsonRoot["targets"].isArray()) {
        LOG_ERROR("[targets] node is not array in targets string: targets: %s, data: %s, userid: %d, client: %s",
    ...
    LOG_INFO("send to client, cmd=msg_type_multichat, targets: %s, data: %s, from userid: %d, from client: %s",
             targets.c_str(), data.c_str(), m_userinfo.userid, conn->peerAddress().toIpPort().c_str());
}
                                       ...
                                       const std::shared_ptr<TcpConnection>& conn)
{
    ...
    ChatServer& imserver = Singleton<ChatServer>::Instance();
    // 单聊消息
    if (targetid >= GROUPID_BOUBDARY) return;
                                           ...
                                           const std::string& oldTeamName, const std::shared_ptr<TcpConnection>& conn)
{
    if (operationType < updateteaminfo_operation_add || operationType > updateteaminfo_operation_modify) {
        LOG_ERROR("invalid teaminfo, userid: %d, , client: %s", m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    if (!Singleton<UserManager>::Instance().getTeamInfoByUserId(m_userinfo.userid, teaminfo)) {
        LOG_ERROR("GetTeamInfoByUserId failed, userid: %d, client: %s", m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        // TODO: 应该应答一下客户端
        return;
    }
    ...
    if (!ok || errs.size() != 0) {
        // TODO: 应该应答一下客户端
        LOG_ERROR("parse teaminfo json failed, userid: %d, client: %s", m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        delete reader;
        return;
    }
    ...
    if (operationType == updateteaminfo_operation_add) {
        ...
        for (uint32_t i = 0; i < teamCount; ++i) {
            if (!jsonRoot[i]["teamname"].isNull() && jsonRoot[i]["teamname"].asString() == newTeamName) {
                // TODO: 提示客户端分组已经存在
                LOG_ERROR("teamname not exist, userid: %d, client: %s", m_userinfo.userid,
                          conn->peerAddress().toIpPort().c_str());
                return;
            }
        }
        ...
        for (uint32_t i = 0; i < teamCount; ++i) {
            if (!jsonRoot[i]["teamname"].isNull() && jsonRoot[i]["teamname"].asString() == oldTeamName) {
                ...
                if (!Singleton<UserManager>::Instance().deleteTeam(m_userinfo.userid, oldTeamName)) {
                    LOG_ERROR("Delete team error, oldTeamName: %s, userid: %s, client: %s", oldTeamName.c_str(),
                              m_userinfo.userid, conn->peerAddress().toIpPort().c_str());
                    return;
                }
    ...
    // 修改分组名
        ...
        if (!Singleton<UserManager>::Instance().modifyTeamName(m_userinfo.userid, newTeamName, oldTeamName)) {
            LOG_ERROR("Update team info failed, userid: %d, newTeamInfo: %s, oldTeamInfo: %s, client: %s",
                      m_userinfo.userid, newTeamInfo.c_str(), oldTeamName.c_str(),
                      conn->peerAddress().toIpPort().c_str());
            return;
        }
    ...
    if (!Singleton<UserManager>::Instance().updateUserTeamInfoInDbAndMemory(m_userinfo.userid, newTeamInfo)) {
        // TODO: 失败应答客户端
        LOG_ERROR("Update team info failed, userid: %d, , newTeamInfo: %s, , client: %s", m_userinfo.userid,
                                           ...
                                           const std::shared_ptr<TcpConnection>& conn)
{
    if (!Singleton<UserManager>::Instance().updateMarknameInDb(m_userinfo.userid, friendid, newmarkname)) {
        // TODO: 失败应答客户端
        LOG_ERROR("Update markname failed, userid: %d, friendid: %d, client: %s", m_userinfo.userid, friendid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
                                                  ...
                                                  const std::shared_ptr<TcpConnection>& conn)
{
    if (newteamname.empty() || oldteamname.empty() || newteamname == oldteamname) {
        LOG_ERROR(
            "Failed to move to other team, newteamname or oldteamname is invalid, userid: %d, friendid:%d, client: %s",
    ...
    if (!Singleton<UserManager>::Instance().isFriend(m_userinfo.userid, friendid)) {
        LOG_ERROR("Failed to move to other team, not your friend, userid: %d, friendid: %d, client: %s",
                  m_userinfo.userid, friendid, conn->peerAddress().toIpPort().c_str());
        // TODO: 通知客户端
        return;
    }
    ...
    if (!Singleton<UserManager>::Instance().getUserInfoByUserId(m_userinfo.userid, currentUser)) {
        LOG_ERROR("User not exist in memory, userid: %d", m_userinfo.userid);
        // TODO: 通知客户端
        return;
    }
    ...
    bool ok = reader->parse(teaminfo.c_str(), teaminfo.c_str() + teaminfo.length(), &jsonRoot, &errs);
    if (!ok || errs.size() != 0) {
        LOG_ERROR("parse teaminfo json failed, userid: %d, teaminfo: %s, client: %s", m_userinfo.userid,
                  teaminfo.c_str(), conn->peerAddress().toIpPort().c_str());
        delete reader;
        return;
    }
    delete reader;
    ...
    if (!foundNewTeam || !foundOldTeam) {
        LOG_ERROR(
            "Failed to move to other team, oldTeamName or newTeamName not exist, userid: %d, friendid: %d, "
            "oldTeamName: %s, newTeamName: %s, client: %s",
    ...
    if (!Singleton<UserManager>::Instance().moveFriendToOtherTeam(m_userinfo.userid, friendid, newteamname)) {
        LOG_ERROR("Failed to MoveFriendToOtherTeam, db operation error, userid: %d, friendid: %d, client: %s",
                  m_userinfo.userid, friendid, conn->peerAddress().toIpPort().c_str());
        return;
    }
...
void ChatSession::deleteFriend(const std::shared_ptr<TcpConnection>& conn, int32_t friendid)
{
    ...
    **/
    ...
    if (!Singleton<UserManager>::Instance().releaseFriendRelationshipInDBAndMemory(friendid, m_userinfo.userid)) {
        LOG_ERROR("Delete friend error, friendid: %d, userid: %d, client: %d", friendid, m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    if (!Singleton<UserManager>::Instance().getUserInfoByUserId(friendid, cachedUser)) {
        LOG_ERROR("Delete friend - Get user error, friendid: %d, userid: %d, client: %s", friendid, m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    if (!Singleton<UserManager>::Instance().updateUserRelationshipInMemory(m_userinfo.userid, friendid,
        ...
        LOG_ERROR("UpdateUserTeamInfo failed, friendid: %d, userid: %d, client: %s", friendid, m_userinfo.userid,
                  conn->peerAddress().toIpPort().c_str());
        return;
    }
    ...
    for (const auto& iter : friends) {
        ...
        if (!targetSessions.empty()) {
            for (auto& iter2 : targetSessions) {
                if (iter2) iter2->sendUserStatusChangeMsg(friendid, 3);
            }
        }
    }
}
...
bool ChatSession::modifyChatMsgLocalTimeToServerTime(const std::string& chatInputJson, std::string& chatOutputJson)
{
    ...
    */
    if (chatInputJson.empty()) return false;
    ...
    if (!ok || errs.size() != 0) {
        LOG_ERROR("parse chatInputJson json failed, userid: %d, chatInputJson: %s", m_userinfo.userid,
                  chatInputJson.c_str());
        delete reader;
        return false;
    }
...
void ChatSession::checkHeartbeat(const std::shared_ptr<TcpConnection>& conn)
{
    ...
    // LOG_INFO("check heartbeat, userid: %d, clientType: %d, client address: %s", m_userinfo.userid,
    // m_userinfo.clienttype, conn->peerAddress().toIpPort().c_str());

    if (time(NULL) - m_lastPackageTime < MAX_NO_PACKAGE_INTERVAL) return;

    conn->forceClose();
    // LOG_INFO("in max no-package time, no package, close the connection, userid: %d, clientType: %d, client address:
    // %s", m_userinfo.userid, m_userinfo.clienttype, conn->peerAddress().toIpPort().c_str());
}