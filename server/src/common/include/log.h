...
#define LOG_FATAL(...)      WhispLog::get_instance().log_output(LOG_LEVEL_FATAL, __FILE__, __LINE__, __VA_ARGS__)        //为了让FATAL级别的日志能立即crash程序，采取同步写日志的方法
#define LOG_CRITICAL(...)   WhispLog::get_instance().log_output(LOG_LEVEL_CRITICAL, __FILE__, __LINE__, __VA_ARGS__)     //关键信息，无视日志级别，总是输出
...
public:
    ...
    bool log_init(const char* log_file_name = nullptr, bool truncate_flag = false, u_int64_t roll_size = 10 * 1024 * 1024);
