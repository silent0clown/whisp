...
#define LOG_TRACE(...)                                                         \
  WhispLog::get_instance().log_output(LOG_LEVEL_TRACE, __FILE__, __LINE__,     \
                                      __VA_ARGS__)
#define LOG_DEBUG(...)                                                         \
  WhispLog::get_instance().log_output(LOG_LEVEL_DEBUG, __FILE__, __LINE__,     \
                                      __VA_ARGS__)
#define LOG_INFO(...)                                                          \
  WhispLog::get_instance().log_output(LOG_LEVEL_INFO, __FILE__, __LINE__,      \
                                      __VA_ARGS__)
#define LOG_WARN(...)                                                          \
  WhispLog::get_instance().log_output(LOG_LEVEL_WARNING, __FILE__, __LINE__,   \
                                      __VA_ARGS__)
#define LOG_ERROR(...)                                                         \
  WhispLog::get_instance().log_output(LOG_LEVEL_ERROR, __FILE__, __LINE__,     \
                                      __VA_ARGS__)
#define LOG_SYSERROR(...)                                                      \
  WhispLog::get_instance().log_output(LOG_LEVEL_SYSERROR, __FILE__, __LINE__,  \
                                      __VA_ARGS__)
#define LOG_FATAL(...)                                                         \
  WhispLog::get_instance().log_output(                                         \
      LOG_LEVEL_FATAL, __FILE__, __LINE__,                                     \
      __VA_ARGS__) // 为了让FATAL级别的日志能立即crash程序，采取同步写日志的方法
#define LOG_CRITICAL(...)                                                      \
  WhispLog::get_instance().log_output(                                         \
      LOG_LEVEL_CRITICAL, __FILE__, __LINE__,                                  \
      __VA_ARGS__) // 关键信息，无视日志级别，总是输出

// 用于输出数据包的二进制格式
#define WHISP_LOG_DEBUG_BIN(buf, buflength)                                    \
  WhispLog::outputBinary(buf, buflength)
...
public:
  ...
  bool log_output(LOG_LEVEL levelv, const char *file_name, int line_num,
                  const char *fmt, ...);
...
private:
  ...
  char *form_log(int &index, char *buf, size_t buf_size, unsigned char *buffer,
                 size_t size);
