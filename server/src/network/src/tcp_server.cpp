...
void TcpServer::newConnection(int sockfd, const InetAddress& peerAddr)
{
    ...
    LOG_DEBUG("TcpServer::newConnection [%s] - new connection [%s] from %s", m_name.c_str(), connName.c_str(),
...
{
    m_loop->assertInLoopThread();
    LOG_DEBUG("TcpServer::removeConnectionInLoop [%s] - connection %s", m_name.c_str(), conn->name().c_str());
    size_t n = m_connections.erase(conn->name());
    //(void)n;
    // assert(n == 1);
    if (n != 1) {
        // 出现这种情况，是TcpConneaction对象在创建过程中，对方就断开连接了。
        LOG_DEBUG("TcpServer::removeConnectionInLoop [%s] - connection %s, connection does not exist.", m_name.c_str(),
                  conn->name().c_str());
        return;
    }
