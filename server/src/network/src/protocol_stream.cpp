...
{
    do {
        unsigned char c = static_cast<unsigned char>(value & 0x7F);
        value >>= 7;
        if (value) c |= 0x80;

        buf.append(1, static_cast<char>(c));
    } while (value);
}
...
BinaryStreamReader::BinaryStreamReader(const char* ptr_, size_t len_) : ptr(ptr_), len(len_), cur(ptr_)
{
...
bool BinaryStreamReader::ReadLengthWithoutOffset(size_t& headlen, size_t& outlen)
{
    ...
    unsigned int value;
    read7BitEncoded(buf, static_cast<uint32_t>(headlen), value);
    outlen = value;
