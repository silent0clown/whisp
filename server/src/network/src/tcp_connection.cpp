...
void network::defaultConnectionCallback(const TcpConnectionPtr& conn)
{
    LOG_DEBUG("%s -> is %s", conn->localAddress().toIpPort().c_str(), conn->peerAddress().toIpPort().c_str(),
              (conn->connected() ? "UP" : "DOWN"));
    // do not call conn->forceClose(), because some users want to register message callback only.
}

void network::defaultMessageCallback(const TcpConnectionPtr&, ByteBuffer* buf, Timestamp)
{
    buf->retrieveAll();
}
...
{
    LOG_DEBUG("TcpConnection::dtor[%s] at 0x%x fd=%d state=%s", m_name.c_str(), this, m_channel->fd(), stateToString());
...
void TcpConnection::send(const void* data, int len)
{
    if (m_state == kConnected) {
        if (m_loop->isInLoopThread()) {
            ...
            string message(static_cast<const char*>(data), static_cast<size_t>(len));
            m_loop->runInLoop(std::bind(static_cast<void (TcpConnection::*)(const string&)>(&TcpConnection::sendInLoop),
                                        this, // FIXME
                                        message));
        }
    }
...
void TcpConnection::sendInLoop(const void* data, size_t len)
{
    ...
    if (!m_channel->isWriting() && m_outputBuffer.readableBytes() == 0) {
        nwrote = sockets::write(m_channel->fd(), data, static_cast<uint32_t>(len));
...
void TcpConnection::handleRead(Timestamp receiveTime)
{
    ...
    if (n > 0) {
        // messageCallback_指向CTcpSession::OnRead(const std::shared_ptr<TcpConnection>& conn, Buffer* pBuffer,
...
void TcpConnection::handleWrite()
{
    m_loop->assertInLoopThread();
    if (m_channel->isWriting()) {
        int32_t n = sockets::write(m_channel->fd(), m_outputBuffer.peek(),
                                   static_cast<uint32_t>(m_outputBuffer.readableBytes()));
        if (n > 0) {
        ...
        LOG_DEBUG("Connection fd = %d  is down, no more writing", m_channel->fd());
    }
}
...
void TcpConnection::handleClose()
{
    ...
    m_loop->assertInLoopThread();
    LOG_DEBUG("fd = %d  state = %s", m_channel->fd(), stateToString());
