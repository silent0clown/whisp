...
bool UserManager::init(const char* dbServer, const char* dbUserName, const char* dbPassword, const char* dbName)
{
    ...
    if (dbPassword != NULL) m_strDbPassword = dbPassword;
    m_strDbName = dbName;

    // 从数据库中加载所有用户信息
    if (!loadUsersFromDb()) return false;

    // TODO: 当用户比较多，这个循环比较慢，优化之
    for (auto& iter : m_allCachedUsers) {
        if (!loadRelationshipFromDb(iter.userid, iter.friends)) {
            LOG_ERROR("Load relationship from db error, userid: %d", iter.userid);
            continue;
        }
    }
...
bool UserManager::loadUsersFromDb()
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName)) {
        LOG_FATAL(
            "UserManager::LoadUsersFromDb failed, please check params: dbserver: %s, dbusername: %s, , dbpassword: %s, "
            "dbname: %s",
    ...
    QueryResult* pResult = pConn->query(
        "SELECT f_user_id, f_username, f_nickname, f_password,  f_facetype, f_customface, f_gender, f_birthday, "
        "f_signature, f_address, f_phonenumber, f_mail, f_teaminfo FROM t_user ORDER BY  f_user_id DESC");
    if (NULL == pResult) {
        LOG_INFO("UserManager::_Query error, dbname: %s", m_strDbName.c_str());
        return false;
    }
    ...
    while (true) {
        ...
        LOG_INFO("userid: %d, username: %s, password: %s, nickname: %s, signature: %s", u.userid, u.username.c_str(),
                 u.password.c_str(), u.nickname.c_str(), u.signature.c_str());

        // 计算当前最大userid
        if (u.userid < GROUPID_BOUBDARY && u.userid > m_baseUserId) m_baseUserId = u.userid;

        // 计算当前最大群组id
        if (u.userid > GROUPID_BOUBDARY && u.userid > m_baseGroupId) m_baseGroupId = u.userid;
    ...
    LOG_INFO("current base userid: %d, current base group id: %d", m_baseUserId.load(), m_baseGroupId.load());
...
bool UserManager::addUser(User& u)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName)) {
        LOG_ERROR(
            "UserManager::AddUser failed, please check params: dbserver: %s, , dbusername: %s, dbpassword: %s, dbname: "
            "%s",
            m_strDbServer.c_str(), m_strDbUserName.c_str(), m_strDbPassword.c_str(), m_strDbName.c_str());
        return false;
    }
    ...
    char sql[256] = {0};
    snprintf(sql, 256,
             "INSERT INTO t_user(f_user_id, f_username, f_nickname, f_password, f_register_time) VALUES(%d, '%s', "
             "'%s', '%s', NOW())",
...
{
    if (smallUserid == greaterUserid) return false;
    ...
    char sql[512] = {0};
    snprintf(sql, 512,
             "INSERT INTO t_user_relationship(f_user_id1, f_user_id2, f_user1_teamname, f_user2_teamname) VALUES(%d, "
             "%d, '%s', '%s')",
             smallUserid, greaterUserid, DEFAULT_TEAMNAME, DEFAULT_TEAMNAME);
    if (!pConn->execute(sql)) {
        LOG_ERROR("make relationship error, sql: %s, smallUserid: %s, greaterUserid: %s", sql, smallUserid,
                  greaterUserid);
        return false;
    }
...
bool UserManager::releaseFriendRelationshipInDBAndMemory(int32_t smallUserid, int32_t greaterUserid)
{
    ...
    snprintf(sql, 256, "DELETE FROM t_user_relationship WHERE f_user_id1 = %d AND f_user_id2 = %d", smallUserid,
             greaterUserid);
    if (!pConn->execute(sql)) {
        LOG_ERROR("release relationship error, sql: %s, smallUserid: %d, , greaterUserid: %d", sql, smallUserid,
                  greaterUserid);
        return false;
    }

    if (!deleteFriendToUser(smallUserid, greaterUserid)) {
        LOG_ERROR("delete relationship error, smallUserid: %d, , greaterUserid: %d", smallUserid, greaterUserid);
        return false;
    }
...
bool UserManager::addFriendToUser(int32_t userid, int32_t friendid)
{
    ...
    for (auto& iter : m_allCachedUsers) {
        ...
        if (bFound1 && bFound2) return true;
    }
...
bool UserManager::updateUserInfoInDb(int32_t userid, const User& newuserinfo)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName)) {
        LOG_ERROR(
            "UserManager::Initialize db failed,  please check params: dbserver: %s, dbusername: %s, dbpassword: %s, "
            "dbname: %s",
            m_strDbServer.c_str(), m_strDbUserName.c_str(), m_strDbPassword.c_str(), m_strDbName.c_str());
        return false;
    }
    ...
    osSql << "UPDATE t_user SET f_nickname='" << newuserinfo.nickname << "', f_facetype=" << newuserinfo.facetype
          << ", f_customface='" << newuserinfo.customface << "', f_gender=" << newuserinfo.gender
          << ", f_birthday=" << newuserinfo.birthday << ", f_signature='" << newuserinfo.signature << "', f_address='"
          << newuserinfo.address << "', f_phonenumber='" << newuserinfo.phonenumber << "', f_mail='" << newuserinfo.mail
          << "' WHERE f_user_id=" << userid;
    if (!pConn->execute(osSql.str().c_str())) {
        LOG_ERROR("UpdateUserInfo error, sql: %s", osSql.str().c_str());
        return false;
    }

    LOG_INFO("update userinfo successfully, userid: %d, sql: %s", userid, osSql.str().c_str());
    ...
    LOG_ERROR(
        "Failed to update userinfo to db, find exsit user in memory error, m_allCachedUsers.size(): %d, userid: %d, "
        "sql: %s",
...
bool UserManager::modifyUserPassword(int32_t userid, const std::string& newpassword)
{
    ...
    std::ostringstream osSql;
    osSql << "UPDATE t_user SET f_password='" << newpassword << "' WHERE f_user_id=" << userid;
    if (!pConn->execute(osSql.str().c_str())) {
        LOG_ERROR("UpdateUserInfo error, sql: %s", osSql.str().c_str());
        return false;
    }

    LOG_INFO("update user password successfully, userid: %d, sql: %s", userid, osSql.str().c_str());
    ...
    LOG_ERROR(
        "Failed to update user password to db, find no exsit user in memory error, m_allCachedUsers.size(): %d, "
        "userid: %d, sql: %s",
        m_allCachedUsers.size(), userid, osSql.str().c_str());
...
bool UserManager::updateUserTeamInfoInDbAndMemory(int32_t userid, const std::string& newteaminfo)
{
    ...
    std::ostringstream osSql;
    osSql << "UPDATE t_user SET f_teaminfo='" << newteaminfo << "' WHERE f_user_id=" << userid;
    if (!pConn->execute(osSql.str().c_str())) {
        LOG_ERROR("Update Team Info error, sql: %s", osSql.str().c_str());
        return false;
    }

    LOG_INFO("update user teaminfo successfully, userid: %d, sql: %s", userid, osSql.str().c_str());
    ...
    LOG_ERROR(
        "Failed to update user teaminfo to db, find no exsit user in memory error, m_allCachedUsers.size(): %d, "
        "userid: %d, sql: %s",
        m_allCachedUsers.size(), userid, osSql.str().c_str());
...
bool UserManager::deleteTeam(int32_t userid, const std::string& deletedteamname)
{
    ...
    std::ostringstream osSql;
    osSql << "UPDATE t_user_relationship SET f_user1_teamname='" << DEFAULT_TEAMNAME << "' WHERE f_user_id1=" << userid
          << " AND f_user1_teamname='" << deletedteamname << "'";

    if (!pConn->execute(osSql.str().c_str())) {
        LOG_ERROR("DeleteTeam error, sql: %s, userid: %d, deletedteamname: %s", osSql.str().c_str(), userid,
    ...
    osSql << "UPDATE t_user_relationship SET f_user2_teamname='" << DEFAULT_TEAMNAME << "' WHERE f_user_id2=" << userid
        ...
        for (auto& iter : m_allCachedUsers) {
            if (iter.userid == userid) {
                for (auto& iter2 : iter.friends) {
                    if (iter2.teamname == deletedteamname) iter2.teamname = DEFAULT_TEAMNAME;
                }
...
bool UserManager::modifyTeamName(int32_t userid, const std::string& newteamname, const std::string& oldteamname)
{
    ...
    std::ostringstream osSql;
    osSql << "UPDATE t_user_relationship SET f_user1_teamname='" << newteamname << "' WHERE f_user_id1=" << userid
          << " AND f_user1_teamname='" << oldteamname << "'";

    if (!pConn->execute(osSql.str().c_str())) {
        LOG_ERROR("ModifyTeamName error, sql: %s, userid: %d, newteamname: %s, oldteamname: %s", osSql.str().c_str(),
                  userid, newteamname.c_str(), oldteamname.c_str());
        return false;
    }

    osSql.str("");

    osSql << "UPDATE t_user_relationship SET f_user2_teamname='" << newteamname << "' WHERE f_user_id2=" << userid
...

    {
        ...
        for (auto& iter : m_allCachedUsers) {
            if (iter.userid == userid) {
                for (auto& iter2 : iter.friends) {
                    if (iter2.teamname == oldteamname) iter2.teamname = newteamname;
                }
                return true;
            }
        }
...
bool UserManager::updateMarknameInDb(int32_t userid, int32_t friendid, const std::string& newmarkname)
{
    ...
    std::ostringstream osSql;
    if (userid < friendid) {
        osSql << "UPDATE t_user_relationship SET f_user1_markname='" << newmarkname << "' WHERE f_user_id1=" << userid
              << " AND f_user_id2=" << friendid;
    } else {
        osSql << "UPDATE t_user_relationship SET f_user2_markname='" << newmarkname << "' WHERE f_user_id2=" << userid
              << " AND f_user_id1=" << friendid;
    }

    if (!pConn->execute(osSql.str().c_str())) {
        LOG_ERROR("Update Markname error, sql: %s", osSql.str().c_str());
        return false;
    }

    LOG_INFO("update markname successfully, userid: %d, friendid: %d, sql: %s", userid, friendid, osSql.str().c_str());
    ...
    LOG_ERROR(
        "Failed to update markname, find no exsit user in memory error, m_allCachedUsers.size(): %d, userid: %d, "
        "friendid: %d, sql: %s",
...
bool UserManager::moveFriendToOtherTeam(int32_t userid, int32_t friendid, const std::string& newteamname)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName)) {
        LOG_ERROR(
            "UserManager::Initialize db failed, please check params: dbserver: %s, dbusername: %s, dbpassword: %s, "
            "dbname: %s, userid: %d, friendid: %d, newteamname: %s",
    ...
    if (userid < friendid) {
        ...
        osSql << "UPDATE t_user_relationship SET f_user2_teamname='" << newteamname << "' WHERE f_user_id1=" << friendid
              << " AND f_user_id2=" << userid;
    }

    if (!pConn->execute(osSql.str().c_str())) {
        LOG_ERROR("MoveFriendToOtherTeam, sql: %s, userid: %d, friendid: %d, newteamname: %s", osSql.str().c_str(),
                  userid, friendid, newteamname.c_str());
        return false;
    }

    LOG_INFO("MoveFriendToOtherTeam db operation successfully, userid: %d, friendid: %d, sql: %s", userid, friendid,
             osSql.str().c_str());

    // 改变内存中用户的分组信息
    User* u = NULL;
    if (!getUserInfoByUserId(userid, u) || u == NULL) {
        LOG_ERROR("MoveFriendToOtherTeam memory operation error, userid: %d, friendid: %d", userid, friendid);
        return false;
    }
...
bool UserManager::addGroup(const char* groupname, int32_t ownerid, int32_t& groupid)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName)) {
        LOG_ERROR(
            "UserManager::AddGroup failed, please check params: dbserver: %s, dbusername: %s, dbname: %s, dbpassword: "
            "%s, groupname: %s, ownerid: %d",
    ...
    char sql[256] = {0};
    snprintf(sql, 256,
             "INSERT INTO t_user(f_user_id, f_username, f_nickname, f_password, f_owner_id, f_register_time) "
             "VALUES(%d, '%d', '%s', '', %d,  NOW())",
             m_baseGroupId.load(), m_baseGroupId.load(), groupname, ownerid);
    if (!pConn->execute(sql)) {
        LOG_ERROR("insert group error, sql: %s", sql);
        return false;
    }
...
bool UserManager::saveChatMsgToDb(int32_t senderid, int32_t targetid, const std::string& chatmsg)
{
    ...
    if (!pConn->initialize(m_strDbServer, m_strDbUserName, m_strDbPassword, m_strDbName)) {
        LOG_ERROR(
            "UserManager::SaveChatMsgToDb failed, please check params: dbserver: %s, dbusername: %s, dbpassword: %s, "
            "dbname: %s",
            m_strDbServer.c_str(), m_strDbUserName.c_str(), m_strDbPassword.c_str(), m_strDbName.c_str());
        return false;
    }
    ...
    if (!pConn->execute(sql.str().c_str())) {
        LOG_ERROR("UserManager::SaveChatMsgToDb, sql: %s, senderid: %d, targetid: %d, chatmsg: %s", sql.str().c_str(),
...
bool UserManager::loadRelationshipFromDb(int32_t userid, std::list<FriendInfo>& r)
{
    ...
    char sql[256] = {0};
    snprintf(sql, 256,
             "SELECT f_user_id1, f_user_id2, f_user1_markname, f_user2_markname, f_user1_teamname, f_user2_teamname "
             "FROM t_user_relationship WHERE f_user_id1 = %d OR f_user_id2 = %d ",
             userid, userid);
    QueryResult* pResult = pConn->query(sql);
    if (NULL == pResult) {
        LOG_INFO("UserManager::Query error, db: %s", m_strDbName.c_str());
        return false;
    }
    ...
    while (true) {
        ...
        if (teamname1.empty()) teamname1 = DEFAULT_TEAMNAME;
        if (teamname2.empty()) teamname2 = DEFAULT_TEAMNAME;
        FriendInfo fi;
        if (friendid1 == userid) {
            ...
            r.emplace_back(fi);
            LOG_INFO("userid=%d, friendid=%d", userid, friendid2);
        } else {
