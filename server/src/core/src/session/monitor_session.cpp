...
const HelpInfo g_helpInfo[] = {{"help", "show help info"},
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (pos != std::string::npos) {
                ...
                LOG_INFO("recv cmd: %s", substr.c_str());
                // LOG_INFO << "buf: " << substr;
                process(conn, substr);
            } else {
                if (totalsize > 0) pBuffer->retrieve(totalsize);
                return;
            }
...
bool MonitorSession::showOnlineUserList(const std::string& token /* = ""*/)
{
    ...
    if (sessions.empty()) {
        ...
        for (const auto& iter : sessions) {
            os << "sessionid:" << iter->getSessionId() << ",userid:" << iter->getUserId()
            ...
            os << ",clienttype:" << iter->getClientType() << ",status:" << iter->getUserStatus() << ".\n";
        }
    }
...
bool MonitorSession::showSpecifiedUserInfoByID(int32_t userid)
{
    ...
    if (userMgr.getUserInfoByUserId(userid, u)) {
        ostringstream os;
        os << "\"address\":\"" << u.address << "\", \"birthday\":" << u.birthday << ",\"customface\": \""
           << u.customface << "\", \"facetype\":" << u.facetype << ",\"gender\":" << u.gender << ",\"mail\":\""
           << u.mail << "\",\"nickname\":\"" << u.nickname << "\",\"phonenumber\":\"" << u.phonenumber
           << "\",\"signature\":\"" << u.signature << ",\"userid\":" << u.userid << ",\"username\":\"" << u.username
           << "\"" << ", teaminfo:" << u.teaminfo << "\n";
