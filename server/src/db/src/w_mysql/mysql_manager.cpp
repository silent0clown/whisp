...
#include "log.h"
// #include "singleton.h"
// #include "w_mysql/mysql_thrdmgr.h"
...
CMysqlManager::CMysqlManager(void)
{
    ...
    // 1. t_user
    {
        ...
        info.m_mapField["f_customface"] = {"f_customface", "varchar(32) DEFAULT NULL COMMENT '自定义头像名'",
                                           "varchar(32)"};
        ...
        info.m_mapField["f_update_time"] = {
            "f_update_time",
            "timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间'", "timestamp"};
    ...
    // 2. t_user_relationship
    {
        ...
        info.m_mapField["f_user1_teamname"] = {
            "f_user1_teamname", "VARCHAR(32) NOT NULL DEFAULT 'My Friends' COMMENT '用户2在用户1的好友分组名称'",
            "VARCHAR(32)"};
        info.m_mapField["f_user2_teamname"] = {
            "f_user2_teamname", "VARCHAR(32) NOT NULL DEFAULT 'My Friends' COMMENT '用户1在用户2的好友分组名称'",
            "VARCHAR(32)"};
        info.m_mapField["f_user1_markname"] = {"f_user1_markname", "VARCHAR(32) COMMENT '用户2在用户1的备注名称'",
                                               "VARCHAR(32)"},
        info.m_mapField["f_user2_markname"] = {"f_user2_markname", "VARCHAR(32) COMMENT '用户1在用户2的备注名称'",
                                               "VARCHAR(32)"},
        info.m_mapField["f_remark"]      = {"f_remark", "varchar(64) NULL COMMENT '备注'", "varchar(64)"};
        info.m_mapField["f_update_time"] = {
            "f_update_time",
    ...
    } else // 检查字段是否匹配， 暂时只检查是否存在， 还需进一步看类型是否需要修改
    {
        ...
        for (std::map<std::string, STableField>::const_iterator it = table.m_mapField.begin();
            ...
            if (mapOldTable.find(field.m_strName) == mapOldTable.end()) {
                ...
                _ss << "alter table " << table.m_strName << " add column " << field.m_strName << " " << field.m_strType;

                std::string sql = _ss.str();
...
bool CMysqlManager::createTable(const STableInfo& table)
{
    ...
    for (std::map<std::string, STableField>::const_iterator it = table.m_mapField.begin(); it != table.m_mapField.end();
         ++it) {
