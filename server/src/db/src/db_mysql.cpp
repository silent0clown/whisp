...

    {
        ...
        if (iTempRet) {
            ...
            if (CR_SERVER_GONE_ERROR == uErrno) {
                ...
                iTempRet = mysql_real_query(m_Mysql, sql, strlen(sql));
                if (iTempRet) {
                    LOG_ERROR("sql error: %s, sql: %s", mysql_error(m_Mysql), sql);
                    // LOG_ERROR << "query ERROR: " << mysql_error(m_Mysql);
                }
            } else {
                // LOG_ERROR << "SQL: " << sql;
...
{
    return static_cast<uint32_t>(mysql_insert_id(m_Mysql));
}
...
int32_t CDatabaseMysql::escapeString(char* szDst, const char* szSrc, uint32_t uSize)
{
    ...
    return static_cast<int32_t>(mysql_real_escape_string(m_Mysql, szDst, szSrc, static_cast<unsigned long>(uSize)));
}