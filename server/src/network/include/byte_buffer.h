...
class ByteBuffer {
   public:
    ...
    {
        if (peek() > start) return nullptr;

        if (start > beginWrite()) return nullptr;
    ...
    {
        if (peek() > end) return false;

        if (end > beginWrite()) return false;
    ...
    {
        uint64_t be64 = sockets::hostToNetwork64(static_cast<uint64_t>(x));
    ...
    {
        uint32_t be32 = sockets::hostToNetwork32(static_cast<uint32_t>(x));
    ...
    {
        if (readableBytes() < sizeof(int64_t)) return -1;
    ...
    {
        if (readableBytes() < sizeof(int32_t)) return -1;
    ...
    {
        if (readableBytes() < sizeof(int16_t)) return -1;
    ...
    {
        if (readableBytes() < sizeof(int8_t)) return -1;
    ...
    {
        if (len > prependableBytes()) return false;
   ...
   private:
    ...
    void makeSpace(size_t len)
    {
        ...
        if (writableBytes() + prependableBytes() < len + kCheapPrepend) {
            ...
            if (kCheapPrepend >= m_readerIndex) return;
